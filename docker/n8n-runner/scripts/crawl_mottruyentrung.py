"""Crawl truyện 'Trở Về Tự Do' từ mottruyentrung.net.vn.

Site dùng theme Madara (WordPress). Danh sách chương lấy qua AJAX endpoint
`/manga/<slug>/ajax/chapters/` (POST). Nội dung mỗi chương nằm trong
`<div class="reading-content"> ... <div class="text-left"> <p>...</p>`.
...
BASE_URL = "https://mottruyentrung.net.vn"
DEFAULT_SLUG = "tro-ve-tu-do"
USER_AGENT = (
    "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 "
    "(KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36"
)

# Site phục vụ cert *.vnnic.vn (hostname không khớp) → tắt verify cho crawler.
# Chỉ áp dụng cho crawl HTML public, không ảnh hưởng MinIO/vLLM (HTTP nội bộ).
...
class ChapterListParser(HTMLParser):
    """Thu thập (title, href) trong các <li class="wp-manga-chapter"><a ...>."""

    def __init__(self) -> None:
        super().__init__()
        self.in_chapter_li = 0
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        attr = dict(attrs)
        if tag == "li" and "wp-manga-chapter" in (attr.get("class") or ""):
            self.in_chapter_li += 1
        elif tag == "a" and self.in_chapter_li and self.current_href is None:
            self.current_href = attr.get("href") or ""
            self.buffer = []

    def handle_endtag(self, tag: str) -> None:
        if tag == "a" and self.in_chapter_li and self.current_href is not None:
            title = " ".join("".join(self.buffer).split())
            if title and self.current_href:
                self.chapters.append((title, self.current_href))
            self.current_href = None
            self.buffer = []
        elif tag == "li" and self.in_chapter_li:
            self.in_chapter_li -= 1

    def handle_data(self, data: str) -> None:
        if self.in_chapter_li and self.current_href is not None:
            self.buffer.append(data)


def fetch_chapter_list(slug: str) -> list[tuple[str, str]]:
    url = f"{BASE_URL}/manga/{slug}/ajax/chapters/"
    html_text = http_request(url, method="POST")
...
class ChapterContentParser(HTMLParser):
    """Trích text các <p> bên trong div.reading-content (hoặc text-left)."""
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        if tag in self.SKIP_TAGS:
            self.skip_depth += 1
            return
        if tag == "div" and "reading-content" in self._class_of(attrs):
            self.in_reading += 1
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Crawl truyện từ mottruyentrung.net.vn")
    p.add_argument("--slug", default=DEFAULT_SLUG, help="manga slug, mặc định: tro-ve-tu-do")
    p.add_argument("--out", default="dataset/tro-ve-tu-do", help="thư mục lưu")
